...
export default function Home() {
  ...
  useEffect(() => {
    fetchStories(0)
    supabase.from('stories').select('*', { count: 'exact', head: true }).then(({ count: c }) => setCount(c))
    ...
    <div style={{ minHeight: '100vh', background: BG, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <div style={{ maxWidth: '520px', margin: '0 auto' }}>
        ...
        <div style={{ padding: '20px 20px 16px', borderBottom: `1.5px solid ${INK}15` }}>
          ...
          {count !== null && <p style={{ fontSize: '11px', color: `${INK}44`, marginTop: '6px', letterSpacing: '0.04em' }}>{count.toLocaleString()} confessions filed</p>}
        </div>
